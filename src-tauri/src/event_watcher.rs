...
fn emit_active_status(app: &AppHandle, ev: &HookEvent, status: &str) {
    ...
    // Web Push：仅 input_required/done 才查标签并推送（去重在 on_task_status 内）。
    if status == "input_required" || status == "done" {
        let label = tm.task_labels.lock().get(&ev.task_id).cloned();
        crate::push::on_task_status(&ev.task_id, status, label.as_deref());
    }
}
