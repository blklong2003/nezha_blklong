...
fn finalize_task_exit(
    ...
    // 优先检查暂停标记——暂停的任务保留会话，不清除附件，不发送 done/failed
    {
        let tm = app.state::<TaskManager>();
        if tm.paused_tasks.lock().remove(task_id) {
            ...
            match reader.read(&mut buf) {
                ...
                Ok(n) => {
                    ...
                    if valid_len > 0 {
                        ...
                        // Write into remote-panel output ring buffer (max 600 chunks per task)
                        {
                            let tm = app.state::<crate::TaskManager>();
                            let mut map = tm.task_output.lock();
                            let entry = map.entry(id.clone())
                                .or_insert_with(|| (0, std::collections::VecDeque::new()));
                            let seq = entry.0;
                            entry.0 += 1;
                            entry.1.push_back((seq, data.clone()));
                            if entry.1.len() > 600 {
                                entry.1.pop_front();
                            }
                        }
...
pub async fn run_task(
    ...
    // Store short label for remote panel display
    let label: String = prompt.lines().next().unwrap_or("").trim().chars().take(55).collect();
    let label = if label.is_empty() { prompt.chars().take(55).collect() } else { label };
    task_manager.task_labels.lock().insert(task_id.clone(), label);
    task_manager.task_output.lock().remove(&task_id);
    ...
    // provider_id 指定时，从 cc-switch 读取对应 provider 的 env 变量注入进程，
    // 优先级高于 launch.extra_env，实现同时运行任务使用不同服务商。
    if let Some(ref pid) = provider_id {
        if let Some(provider_env) = crate::cc_switch::get_provider_env(pid, &agent) {
...
/// 暂停运行中的任务：杀进程、不清理会话、前端可 resume 恢复。
/// 退出监视器在 finalize_task_exit 中通过 paused_tasks 判断跳过清理。
#[tauri::command]
pub async fn pause_task(
    ...
    // 标记为已暂停，退出监视器看到此标记会跳过清理
    task_manager.paused_tasks.lock().insert(task_id.clone());
    task_manager.cancelled_tasks.lock().remove(&task_id);
    task_manager
    ...
    if let Some(arc) = child_arc {
        ...
        // 不删除 child_handles/sessions——让退出监视器处理清理，
        // 它在 finalize_task_exit 中已加入 paused_tasks 判断
    }

    let _ = app.emit(
        "task-status",
        serde_json::json!({ "task_id": task_id, "status": "interrupted" }),
    );
...
pub async fn resume_task(
    ...
    task_manager.cancelled_tasks.lock().remove(&task_id);
    task_manager.manually_completed_tasks.lock().remove(&task_id);
    // Clear stale output buffer so resumed task starts fresh in remote panel
    task_manager.task_output.lock().remove(&task_id);
