...
pub(crate) fn emit_task_status(app: &AppHandle, task_id: &str, status: &str) {
    ...
    // Web Push：仅在需要关注/完成时才查标签并推送，避免高频 running 转换上的额外加锁。
    if status == "input_required" || status == "done" {
        let label = app
            ...
            .get(task_id)
            .cloned();
        crate::push::on_task_status(task_id, status, label.as_deref());
    }
}
...
/// 共享解析器：桌面命令 `read_session_messages` 与远程面板 `/api/task/<id>/messages`
/// 都调用它，保证两端返回**完全相同**的 `SessionMessage[]` 结构（单一契约，防前后端漂移）。
pub(crate) fn parse_session_messages_file(session_path: &str) -> Result<Vec<SessionMessage>, String> {
    use std::collections::VecDeque;

    // 流式逐行读取，避免把整个（可能数百 MB 的）会话文件一次性载入内存。
    //  - head：仅用于格式判定（codex 的 session_meta 在文件开头）。
    //  - tail：保留最近 MAX_LINES 行用于解析，内存有界。两个解析器均逐行独立，
    //    只解析尾部行不影响内容正确性；超长会话只展示最近部分。
    const MAX_LINES: usize = 20_000;
    let file = File::open(session_path).map_err(|e| e.to_string())?;
    let reader = BufReader::new(file);

    let mut head: Vec<String> = Vec::with_capacity(16);
    ...
    if is_codex_format(&head_refs) {
        Ok(parse_codex_session(&tail_refs))
    } else {
        Ok(parse_claude_session(&tail_refs))
    }
}
...
mod tests {
    ...
    /// 契约测试：锁定 SessionMessage/SessionContent 的序列化 JSON 结构，
    /// 必须与前端 src/components/session-view/types.ts 的 TS 类型一致。
    /// 桌面(invoke) 与远程面板(/api) 复用此结构——任一端改字段，此测试即红。
    #[test]
    fn session_message_serializes_to_frontend_contract() {
